...
class UserModel
{
    ...
    public static function createNewUser($data)
    {
       ...
       */
        $db = new Database();
        $sql = "INSERT INTO user(userid,username,password) VALUES('".$db->escapeString($data['userid'])."','".$db->escapeString($data['username'])."','".$db->escapeString($data['password'])."')";
        $db->query($sql);
        $data['id'] = $db->insertId();
        return (object) $data;
    }
    public static function saveUser($data)
    {
       /* $db = new Database();
        ...
        $db = new Database();
        $sql = "UPDATE user SET username='".$db->escapeString($data['username'])."',password='".$db->escapeString($data['password'])."' WHERE id=".intval($data['id']);
        $db->query($sql);
        return (object) $data;
    }
